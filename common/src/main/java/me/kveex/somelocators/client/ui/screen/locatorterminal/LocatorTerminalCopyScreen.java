...
public class LocatorTerminalCopyScreen extends LocatorTerminalRelatedScreen {
    ...
    private void initUI(ItemStack stackFrom, ItemStack stackTo, boolean showRewriteWarning) {
        ...
        Optional<LocatorComponent> locatorComponent = Optional.ofNullable(stackFrom.get(ModComponents.LOCATOR_COMPONENT.get()));
        Optional<LodestoneTracker> compassComponent = Optional.ofNullable(stackFrom.get(DataComponents.LODESTONE_TRACKER));
        if (locatorComponent.isEmpty() && compassComponent.isEmpty()) return;
        ...
        Component copyButtonText = showRewriteWarning
        ...
        this.copyButton = ButtonElement.builder(copyButtonText, button -> {
            var selectedPoints = list.getSelectedPoints();

            if (stackTo.is(Items.COMPASS)) {
                stackTo.set(DataComponents.LODESTONE_TRACKER, new LodestoneTracker(Optional.ofNullable(points.getFirst().target()), true));
            } else stackTo.set(ModComponents.LOCATOR_COMPONENT.get(), new LocatorComponent(selectedPoints.getFirst(), selectedPoints, false));
